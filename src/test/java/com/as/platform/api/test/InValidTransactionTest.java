...
public class InValidTransactionTest extends TestBase {
    ...
    public void createTransaction() {
        BigDecimal amount = TestDataGenerator.randomBigDecimal(Double.parseDouble(testContext.getAttribute("from_balance").toString()), 100000, 0);
    ...
    public void cleanup() {
        DeleteAccountResultModel deleteFromAccountResultModel = postmanClient.deleteAccountById(
                ...
                Header.builder().apiKey(testContext.getAttribute(APIConstants.API_KEY).toString() + "error").build()
        );
